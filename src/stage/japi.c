...
void Back_japi_DrawFG(StageBack *back)
{
	Back_japi *this = (Back_japi*)back;
...
void Back_japi_DrawMD(StageBack *back)
{
	Back_japi *this = (Back_japi*)back;
...
void Back_japi_DrawBG(StageBack *back)
{
	Back_japi *this = (Back_japi*)back;
	...
	static const struct Back_japi_LowerBop
	{
		RECT src;
		RECT_FIXED dst;
	} lbop_piece[] = {
	...
	const struct Back_japi_LowerBop *lbop_p = lbop_piece;
...
void Back_japi_Free(StageBack *back)
{
	Back_japi *this = (Back_japi*)back;
...
StageBack *Back_japi_New(void)
{
	//Allocate background structure
	Back_japi *this = (Back_japi*)Mem_Alloc(sizeof(Back_japi));
	if (this == NULL)
		return NULL;
	
	//Set background functions
	this->back.draw_fg = Back_japi_DrawFG;
	this->back.draw_md = Back_japi_DrawMD;
	this->back.draw_bg = Back_japi_DrawBG;
	this->back.free = Back_japi_Free;
	
	//Load background textures
	IO_Data arc_back = IO_Read("\\JAPI\\BACK.ARC;1");
